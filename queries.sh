...
WINGOALS="$($PSQL "SELECT SUM(winner_goals) FROM games")"
OPPGOALS="$($PSQL "SELECT SUM(opponent_goals) FROM games")"
AVEWINGOALS="$($PSQL "SELECT AVG(winner_goals) FROM games")"
NUMGAMES="$($PSQL "SELECT COUNT(*) FROM games")"
TOTALGOALS=$(( WINGOALS + OPPGOALS ))
